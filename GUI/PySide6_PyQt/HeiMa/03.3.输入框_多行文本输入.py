from PySide6.QtWidgets import QApplication, QWidget,QLabel,QLineEdit,QVBoxLayout,QTextEdit
...
def input_box(window):#为什么要加window,因为要设置组件的父对象不加也行
    ...
    #_______________________________多行文本输入框
    text_box=QTextEdit()
    text_box.setPlaceholderText("请输入多行文本")#设置提示文字
    # text_box.setPlainText("这是一个多行文本输入框")#设置默认文字
    text_box.setHtml("<h1>请输入多行文本</h1>")#设置html文本
    layout.addWidget(text_box)#添加到布局
    print(text_box.toPlainText())   #获取输入框内容
