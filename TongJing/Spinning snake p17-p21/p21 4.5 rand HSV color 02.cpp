...
//	int radius = 200;  // 半径
//
//	float offset = 0;//扇形组件角度偏移值
//	float totaloffset = 0;//扇形组件总偏移值
//
//
//	//一个圆的整体循环
//	while (1) {//这个while循环是配合_getch() 按键后颜色会随机变化
//
//		float h = rand() % 180; // 随机生成0-179的整数 做HSV的H值随机颜色用
//		COLORREF color1 = HSVtoRGB(h, 1, 1);//将HSV颜色转换为RGB颜色
//		COLORREF color2 = HSVtoRGB(h + 180, 1, 1);//+180得到互补色
//
...
//				Sleep(sleep);
//				setfillcolor(color1);//HSV色调color1
...
//
//				//Sleep(sleep);
//				setfillcolor(color2);//HSV色调color2
...
//			}
//			totaloffset += Pi / 20;//每次循环结束偏移Pi/20
//		}
//		_getch();	  // 暂停窗口}
//	}
//
